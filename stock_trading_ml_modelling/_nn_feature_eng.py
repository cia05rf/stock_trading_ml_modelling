...
from stock_trading_ml_modelling.config import HIST_PRICES_W, \
    NN_FT_PERIODS, PERIOD_HIGH_VOLATILITY, GAP_HIGH_VOLATILITY, \
    NN_TARGET_PERIODS, NN_FT_NUMPY, NN_TAR_NUMPY
from stock_trading_ml_modelling.utils.timing import ProcessTime
from stock_trading_ml_modelling.utils.ft_eng import norm_time_s, \
    calc_ema_macd, flag_mins, flag_maxs, mk_prev_move_float, mk_prev_move_float, \
    prev_max_min


# import the price data
prices_w = pd.read_hdf(HIST_PRICES_W)
...
def build_ft_arrays(df_in):
    ...
    def pos_fts(col, reverse: bool = False):
        ...
        """
        li = [norm_time_s(x, df_in[col], NN_FT_PERIODS, _return_series=True,
                          _fill_window=True).tolist() for x in df_in.index]
        if reverse:
    ...
    def pos_neg_fts(col):
        ...
        """
        raw_li = [norm_time_s(x, df_in[col], NN_FT_PERIODS, _return_series=True,
                              _fill_window=True, _neg_vals=True).tolist() for x in df_in.index]
        abs_li = [np.abs(x).tolist() for x in raw_li]
        pos_binary_li = [(np.array(x) > 0).astype(int).tolist()
                         for x in raw_li]
        neg_binary_li = [(np.array(x) < 0).astype(int).tolist()
                         for x in raw_li]
    ...
    # Find turn points
    df_in['real_close_min'] = flag_mins(
        df_in['close'], _period=PERIOD_HIGH_VOLATILITY, _cur=False)
    df_in['real_close_max'] = flag_maxs(
        df_in['close'], _period=PERIOD_HIGH_VOLATILITY, _cur=False)
    for max_min in ['max', 'min']:
        # Find the last 2 mins
        df_in["prev_{}_close".format(max_min)], df_in["prev_{}_close_date".format(max_min)], df_in["prev_{}_close_index".format(max_min)] = prev_max_min(
            df_in[["date", 'close', "real_close_{}".format(max_min)]].copy(), 'close', "real_close_{}".format(max_min), GAP_HIGH_VOLATILITY)
        df_in["prev_{}_close_change".format(max_min)] = mk_prev_move_float(
            df_in['prev_{}_close'.format(max_min)])
        df_in["prev_{}_close_index_change".format(max_min)] = mk_prev_move_float(
            df_in['prev_{}_close_index'.format(max_min)])
        # Calc the gradient
        df_in['prev_{}_close_grad'.format(max_min)] = df_in["prev_{}_close_change".format(
            max_min)] / df_in["prev_{}_close_index_change".format(max_min)]
        # Count the periods since the change
        df_in['prev_{}_close_period_count'.format(
            max_min)] = df_in.index - df_in["prev_{}_close_index".format(max_min)]
        # Calc the projected value and diff to the actual value
        df_in['prev_{}_projected_close'.format(max_min)] = df_in["prev_{}_close".format(
            max_min)] + (df_in['prev_{}_close_period_count'.format(max_min)] * df_in['prev_{}_close_grad'.format(max_min)])
        df_in['prev_{}_projected_close_diff'.format(max_min)] = (
            df_in["close"] - df_in['prev_{}_projected_close'.format(max_min)]) / df_in["close"]
    ...
    ### MAX CLOSE GRAD DIFF ###
    # Normalise over time period
    max_cl_grad_diff_li, max_cl_grad_pos_diff_li, max_cl_grad_diff_neg_li = pos_neg_fts(
        'prev_max_projected_close_diff')

    ### MIN CLOSE GRAD DIFF ###
    # Normalise over time period
    min_cl_grad_diff_li, min_cl_grad_diff_pos_li, min_cl_grad_diff_neg_li = pos_neg_fts(
        'prev_min_projected_close_diff')

    ### BUILD TARGET VARIABLE ###
    # Use the value of the EMA12 close n periods from this one relative to this EMA12
    target = np.array(
        (df_in.ema12.shift(-NN_TARGET_PERIODS) / df_in.ema12).tolist())
    ...
    for i in range(len(close_li)):
        ...
        for j in range(len(close_li[i])):
            tmp_li.append([
                close_li[i][j], macd_li[i][j], macd_pos_li[i][j], macd_neg_li[i][j], cl_vs_cl_li[i][j], cl_vs_cl_pos_li[i][j], cl_vs_cl_neg_li[i][j], cl_vs_op_li[i][j], cl_vs_op_pos_li[i][j], cl_vs_op_neg_li[i][j], cl_vs_hi_li[
                    i][j], cl_vs_lo_li[i][j], max_cl_grad_diff_li[i][j], max_cl_grad_pos_diff_li[i][j], max_cl_grad_diff_neg_li[i][j], min_cl_grad_diff_li[i][j], min_cl_grad_diff_pos_li[i][j], min_cl_grad_diff_neg_li[i][j]
            ])
...
tickers = prices_w.ticker.unique()
for tick in tickers:
    print('\nRUNNING LAP {}/{} - TICKER -> {}'.format(len(run_time.lap_li) +
          1, len(tickers), tick))
...
### EXPORT ###
##############
np.save(NN_FT_NUMPY, features)
np.save(NN_TAR_NUMPY, target)
