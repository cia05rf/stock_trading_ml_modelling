...
from utils.date import create_full_year_days, calc_wk_st_date
from libs.logs import NoLog
from scrapping.scrape_data import get_public_holidays
from prices.get_data import all_df, fetch_daily_prices, fetch_weekly_prices

def filter_year_dates(year, year_dates, log=NoLog()):
    ...
    public_holidays = get_public_holidays(year, log=log)
...
def create_filtered_year_dates(year, from_date=None, to_date=None, log=NoLog()):
    ...
    year_dates = filter_year_dates(year, year_dates, log=log)
    return year_dates

#Create a weekly table
def daily_to_weekly_price_conversion(dp_df, log=NoLog()):
...
def split_week_prices(ticker_ids=[], from_date=None, to_date=None, log=NoLog()):
    ...
    _, new_wp_df = daily_to_weekly_price_conversion(dp_df, log=log)
